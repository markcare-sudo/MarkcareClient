...
import React, { useEffect, useState } from "react";
import { useParams, Link } from "react-router-dom";
import { ArrowLeft, Calendar, Clock, Share2, Bookmark, CheckCircle2, Tag as TagIcon } from "lucide-react";
import { motion, useScroll, useSpring } from "framer-motion";
import { useGlobalContext } from "@/context/GlobalContext";
...
export default function BlogDetails() {
  ...
  useEffect(() => {
    ...
    const loadBlog = async () => {
      try {
        ...
        // 🛠 FIX: Drill down to response.data.data based on your JSON structure
        const blogData = response?.data?.data || response?.data || response;
  ...
  const image = blog?.featured_media ? getImageUrl(blog.featured_media) : DUMMY_IMAGE;
  const formattedDate = blog?.published_at
    ? new Date(blog.published_at).toLocaleDateString("en-IN", { year: "numeric", month: "long", day: "numeric" })
    : "Draft Edition";

  // Calculate dynamic reading time (approx 200 words per min)
  const readingTime = Math.ceil((blog.content?.split(/\s+/).length || 0) / 200) || 5;
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-100 selection:bg-red-500/30">
      ...
      {/* 1. HERO SECTION */}
      <header className="relative w-full h-[85vh] flex items-end overflow-hidden">
        <motion.div 
          initial={{ scale: 1.1, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ duration: 1.2 }}
        ...
        <div className="container mx-auto px-6 pb-16 relative z-10 max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            ...
            <div className="flex items-center gap-3 mb-6">
              {blog.category && (
                <span className="px-4 py-1.5 text-[10px] font-black tracking-widest uppercase bg-red-600 text-white rounded-full">
                  {blog.category}
                </span>
              )}
              <span className="h-px w-8 bg-white/20" />
              <div className="flex items-center gap-2 text-gray-400 text-[10px] uppercase font-bold tracking-widest">
                <Clock size={12} className="text-red-500" /> {readingTime} MIN READ
              </div>
            </div>
            
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold leading-[1.1] text-white mb-8 tracking-tighter max-w-4xl">
              {blog.title}
            </h1>

            <div className="flex items-center gap-4 text-gray-400 text-sm border-t border-white/10 pt-8 w-fit">
               <div className="w-10 h-10 rounded-full bg-neutral-800 flex items-center justify-center font-bold text-red-500 border border-white/10">MC</div>
               <div className="flex flex-col">
                  <span className="text-white font-bold">MarkCare Editorial</span>
                  <span className="text-xs opacity-60">{formattedDate}</span>
               </div>
      ...
      <main className="container mx-auto px-6 py-20 max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-8">
            {blog.excerpt && (
              <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative mb-16">
                <div className="absolute -left-6 top-0 bottom-0 w-1 bg-red-600" />
                <p className="text-2xl text-neutral-200 font-medium leading-relaxed italic">
                  "{blog.excerpt}"
                </p>
              </motion.div>
            )}
            ...
            <article
              className="prose prose-invert prose-lg md:prose-xl max-w-none 
                prose-p:text-neutral-400 prose-p:leading-[1.8] prose-p:mb-8
                prose-headings:font-bold prose-headings:text-white prose-headings:tracking-tight
                prose-strong:text-white prose-a:text-red-500 prose-img:rounded-2xl"
              dangerouslySetInnerHTML={{ __html: blog.content || "" }}
            />

            {/* 🛠 FIX: Tags access changed from blog.Tags to blog.tags */}
            {blog.tags?.length > 0 && (
              <div className="mt-20 pt-10 border-t border-white/5">
                <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 mb-6 flex items-center gap-2">
                   <TagIcon size={12} /> Related Topics
                </h4>
                <div className="flex flex-wrap gap-2">
                  {blog.tags.map((tag) => (
                    <Link
                      key={tag.id}
                      to={`/blogs?tag=${tag.slug}`}
                      className="text-[10px] font-bold uppercase tracking-widest px-4 py-2 bg-neutral-900 border border-white/5 rounded-lg hover:bg-white hover:text-black transition-all"
                    >
                      {tag.name.replace('#', '')}
                    </Link>
                  ))}
            ...
            <div className="sticky top-24 space-y-8">
              <div className="bg-neutral-900/50 border border-white/5 p-8 rounded-3xl">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2 text-white">
                  <Bookmark size={18} className="text-red-600" /> Story Tools
                </h3>
                <div className="space-y-4">
                  <button onClick={handleShare} className="w-full py-4 bg-red-600 text-white font-bold rounded-xl flex items-center justify-center gap-3 hover:bg-red-700 transition">
                    <Share2 size={18} /> Share Insight
                  </button>
                  <div className="pt-6 mt-6 border-t border-white/5 space-y-4">
                    <div className="flex items-center gap-3 text-xs text-neutral-400">
                      <CheckCircle2 size={14} className="text-red-500" />
                      <span>Verified Industry Data</span>
                    </div>
                  </div>
                </div>
              </div>

              {/* Keyword Cloud (Optional Visual) */}
              {blog.keywords?.length > 0 && (
                <div className="p-6 bg-neutral-900/30 border border-white/5 rounded-3xl">
                   <p className="text-[10px] font-bold uppercase text-neutral-500 mb-4 tracking-widest">Key Terms</p>
                   <div className="flex flex-wrap gap-2">
                     {blog.keywords.map(kw => (
                       <span key={kw.id} className="text-[10px] text-neutral-400 bg-white/5 px-2 py-1 rounded">
                         {kw.keyword}
                       </span>
                     ))}
                   </div>
